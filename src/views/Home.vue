                ...
                <div class="bg-white border-2 border-[#0A0910] rounded-lg rounded-tl-none p-2 md:p-6">
                    ...
                    <div v-if="isLoading || cuisineSlots.length > 0">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div
                                ...
                                :key="cuisineInfo.id"
                                class="border-2 border-[#0A0910] rounded-lg overflow-hidden shadow-brutal"
                    ...
                    <div v-else-if="recipes.length > 0 && cuisineSlots.length === 0" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div
                            ...
                            :key="recipe.id"
                            class="border-2 border-[#0A0910] rounded-lg overflow-hidden shadow-brutal animate-fade-in-up"
...
import { generateCustomRecipe, generateMultipleRecipesStream, generateRecipe, recognizeIngredients } from '@/services/aiService'
...
// 调用 AI 识别图片中的食材（由后端视觉模型完成，避免前端暴露密钥）
const recognizeIngredientsFromImage = async (base64Data: string): Promise<string[]> => {
    try {
        return await recognizeIngredients(base64Data)
    } catch (error) {
